...
{{--                        </div>--}}
    ...
    <div class="modal fade" id="delete" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle">Xóa khuôn bế</h5>
                </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.add').click(function () {
                modal.find('#modal-title').text('Thêm bên khuôn bế')
                $('#name').val('')
                $('#phone').val('')
                $('#address').val('')
                $('#url').val('{{ route('create_mold') }}')
                modal.modal('show')
            })
            ...
            $('.delete').click(function () {
                ...
                let id = $(this).data('id')
                $('#delete-id').val(id)
                $('#delete-text').text(`Chắn chắc xoá bên khuôn bế ${name}?`)
                $('#delete').modal('show')
            })
